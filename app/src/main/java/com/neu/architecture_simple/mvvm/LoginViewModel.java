...
public class LoginViewModel extends ViewModel {
    ...
    public void doLogin(String name, String passwd) {
        ...
        final int code = Objects.requireNonNull(userModel.getValue()).checkUserValidity(name, passwd);
        if (code != 0) isLoginSuccess = false;
        final Boolean result = isLoginSuccess;
        new Handler(Looper.myLooper()).postDelayed(() -> loginStateModel.postValue(result), 5000);
